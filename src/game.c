...
Esse jogo e inspirado no jogo The Binding of Isaac. Esse projeto e apenas para fins de estudos e nao visa qualquer objetivo comercial.
Essa implementacao pode melhorar muito ainda, pois fomos desenvolvendo e melhorando nossas habilidades ao longo do semestre e se fossemos comecar d
o zero, com certeza fariamos melhor e diferente, mas o aprendizado obtido com esse trabalho e o mais valioso.
 */
#include <math.h>
#include <string.h>
#include <ctype.h>
...
#define LADO_QUADRADOX 20 // Largura do personagem
#define LADO_QUADRADOY 20 // Altura do personagem
#define VELOCIDADE 1      // Velocidade que percorre dentro da matriz

#define MAX_TRAPS (MAP_HEIGHT * MAP_WIDTH) / 4         // N maximo de Fogueiras(ou traps): ate 1/4 da area do mapa pode ser trap
#define MAX_BOMBS 20                                   // Numero maximo de bombas que o jogador pode carregar
#define EXP_RADIUS 6                                   // Raio das explosoes das bombas
#define EXP_AREA 3 * EXP_RADIUS *EXP_RADIUS *MAX_BOMBS // Area maxima que as explosoes de todas as bombas somadas podem atingir

#define DEATH_DELAY 2 // Tempo que o personagem fica morto
#define MAX_TIROS 10  // Quantidade maxima de tiros que o jogador pode dar
#define MAX_SCORES 10 // Quantidade maxima de scores exibidas no highscores

#define MAX_INIMIGOS 14  // Quantidade maxima de inimigos (gaper e pooter somados)
#define BOSS_DELAY 20    // Tempo para o boss chamar reforcos
#define SPECIAL_DELAY 10 // delay para o player usar o especial

// Velocidades: Quanto menor o valor, mais rapido:

#define PLAYER_SPEED 0.10 // velocidade do jogdor
#define ENEMY_SPEED 0.11  // velocidade dos inimigos
#define BOSS_SPEED 0.18   // velocidade do boss
#define SHOT_SPEED 0.060  // velocidade dos tiros
/* aqui a velocidade trata-se de um delay em segundos que os personagens tem para realizar o movimento, ja que com 60 fps as execucoes sao muito rapidas e a velocidade dos personagens tambem. Aqui foi util definir velocidades dferentes para inimigos, jogador, boss e tiros, para tornar o jogo mais balanceado */

#define MAX_MAPS 10 // Numero maximo de mapas no modo normal
// Atualizar sempre que acrescentar ou remover um mapa, para que carregue adequadamente ou nao crashe na funcao de manipulacao de arquivo
...
typedef struct TIRO
{
    ...
    int x, y;          // posicoes
    int dx, dy;        // direcoes
    float delay;       // delay que precisa ser esperado a cada tiro
    float speed;       // velocidade dp tiro
    bool active;       // Retorna se esta ativo ou nao
    bool collided;     // Retorna se entidade esta em colisao
    bool isPlayer;     // Retorna se o tiro pertence a um jogador
} TIRO;                // Tiro dos personagens
typedef struct ENTIDADE
{

    TIRO tiro[MAX_TIROS]; // Tiros
    Texture2D texture;    // textura
    Color color;          // cores
    int x, y;             // posicao x e y
    int dx, dy;           // direcoes
    int health;           // Saude (se chega em 0, jogador perde uma vida)
    float delay;          // delay do movimento
    float special_delay;  // delay da habilidade especial
    int lives;            // vidas (se chega em zero acaba o jogo)
    int pontuacao;        // pontuacao
    int n_bombs;          // numero de bombas que a entidade carrega
    int n_tiros;          // numero de tiros que a entidade disparou (serve para controlar a quantidade maxima de tiros)
    int spritex;          // posicao no eixo x da imagem (serve para criar animacoes com as sprites)
    int spritey;          // posicao no eixo y da imagem (serve para criar animacoes com as sprites)
    bool active;          // Indica se esta vivo ou morto
    bool collided;        // Indica se entidade esta em colisao
    bool canChase;        // Indica se entidade pode perseguir um jogador
    bool isPlayer;        // Indica se a entidade e um jogador
    bool isBoss;          // Indica se a entidade e um boss
...
typedef struct ITEM
{
    ...
    int spritex;
    int spritey;
    float alive_time; // variavel de controle do tempo e vida do item (muito util para explodir as bombas)
    bool active;
    bool colectable; // indica se o item e coletavel ou nao
} ITEM;              // Struct para criar os itens coletaveis ou nao do mapa, como bombas e traps

typedef struct TIMER
{
    float death;     // Temporizador que mantem o jogador morto por alguns segundos
    float invecible; // Temporizador que controla o tempo de imunidade do jogador (logo quando morre, inicia uma nova fase ou usa o poder especial)
} TIMER;             // Temporizadores uteis

typedef struct GAME
{                // Struct que contem todas as estruturas necessarias do jogo
    ITEM portal; // portal
    ITEM trap[MAX_TRAPS];
    ITEM bomb[MAX_BOMBS];
    ...
    ENTIDADE boss;                     // boss da fase final
    ENTIDADE tiro[MAX_TIROS];
    int map[MAP_HEIGHT][MAP_WIDTH]; // matriz do mapa do jogo
    int current_map;                // fase atual
    int modo_jogo;                  // 1 e padrao, 2 e a geracao aleatoria

    // variaveis de controle dos vetores dos inimigos, bombas e etc::
    int n_gaper;
    int n_pooter;
    int n_bombas;
    int n_traps;
    int n_explosion;
    int n_tiro;
    char state;     // estados do jogo: usado para fazer as transicoes entre funcoes de menu e jogo
    bool isLoaded;  // verifica se o jogo atual veio de um jogo carregado
    int frameCount; // Conta o tempo decorrido de jogo
} GAME;
...
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------
void movimentarBoss(ENTIDADE *entidade)
{ // Movimenta entidades maiores
    int i, j;
...
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------
void checkCollision(ENTIDADE *ent)
{ // Checa as colisoes entre jogador, inimigos e itens (exceto o portal)
    int i, j;
    ...
    for (i = 0; i < MAX_TIROS; i++)
    {
        ...
        {
            if (!ent->isPlayer)
            { // Player nao morre com proprio tiro
                ent->health -= 60;
...
//------------------------------------------------------------------------------------------------
void checkBombs()
{ // Verifica as bombas ativas no mapa e realiza a explosao
    int tmp = 0;
    int i, j, exp_range;
...
{ // movimenta os projeteis do jogador ou dos inimigos
    int i;
    if (ent->n_tiros == MAX_TIROS)
        ent->n_tiros = 0;
    if (!ent->isPlayer)
    {
...
//------------------------------------------------------------------------------------------------
void finishGame()
{ // Encerra o jogo salvando a pontuacao do jogador
    ...
    // Calcula a pontuacao do jogador levando em conta o tempo decorrido e o numero de vidas (inversamente proporcional ao tempo)
    game.player.pontuacao += (game.player.pontuacao / (0.1 + game.frameCount / 500)) * game.player.lives; // note que se o jogador morre e da game over, o tempo decorrido nao entra para a contagem.
...
{ // Funcao que desenha o jogo
    ...
    for (i = 0; i < game.n_traps; i++)
    {
        ...
        if (game.trap[i].spritex >= 31 * 50)
            game.trap[i].spritex = 0;
    ...
    for (i = 0; i < game.n_bombas; i++)
    {
        if (game.bomb[i].active)
        {
            ...
            if (!game.bomb[i].colectable)
            {
                ...
                if (game.bomb[i].spritex >= 32 * (60 * 3))
                    game.bomb[i].spritex = 0;
    ...
    for (i = 0; i < game.n_explosion; i++)
    {
        ...
        if (game.explosion[i].spritex >= 32 * (60 * 2))
            game.explosion[i].spritex = 0;
    ...
    DrawText(TextFormat("Tempo: %03.2f", (float)game.frameCount / 60), 10, (MAP_HEIGHT * FATORY) + 50, 20, WHITE);
    ...
    DrawText(TextFormat("Especial: %.1f", game.player.special_delay), 10, (MAP_HEIGHT * FATORY) + 150, 20, WHITE);
...
void novo_jogo() // Funcao que carrega um novo jogo (inclusive quando e feito o load do save e quando avanca de fase)
{
    ...
    if (!game.isLoaded)
    { // Reseta o numero de itens e inimigos caso o jogo NAO tenha sido carregado a partir de um save
        game.n_bombas = 0;
        game.n_gaper = 0;
        game.n_pooter = 0;
        game.n_explosion = 0;
    ...
    if (!game.isLoaded)
    {
        ...
        { // spawna o boss em mapas multiplos de 10
    ...
    //Corrige um bug que acontecia nas texturas na hora de carregar o jogo:
    for (i = 0; i < MAX_TIROS; i++) 
        {
            game.player.tiro[i].texture = LoadTexture("../sprites/tears.png");
            game.player.tiro[i].color = RAYWHITE;
    ...
    while (!WindowShouldClose() && game.state != 'q' && game.state != 'n' && game.state != 'p' && game.state != 'g')
    {
        ...
        if (game.modo_jogo || game.current_map != 10)
            game.boss.active = false;
        ...
        if (game.player.health <= 0)
        { // Muda as animacoes e ativa o modo invencivel quando o player morre
            game.player.dx = 0;
            game.player.dy = 0;
            game.player.active = false;
